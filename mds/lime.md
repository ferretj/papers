...
An explanation, for a given example, gives an amplitude and a color to some variables
...
The optim problem is :
 
* minimize approximation of f (model) by g (interpretability model) in a proximity zone of x (weighted by actual proximity)
* WHILE keeping a low complexity for g

**The focus of the paper is on sparse linear models, with search performed using perturbations**
...
The core idea is that, under a budget of n instances to explore, we want to maximize the cumulative importance of features with globally-defined importance (for the instances chosen)
